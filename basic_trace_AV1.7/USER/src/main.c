...
//		}
		...
		// 【修改2：非阻塞的停车阶段 3000ms 等待】
		if(parking_cnt == 3 && is_checking_parking == 1)
		{
			static uint32_t parking_wait_start = 0;
			if(parking_wait_start == 0)
			{
				parking_wait_start = g_millis;
			}
			
			if(g_millis - parking_wait_start > 3000)
			{
				// 注释掉绝对不能用的 g_millis = 0;
				parking_cnt = 4;
				parking_wait_start = 0;
			}
		}
		else if(avoid_cnt == 0 && g_parking_state == PARKING_IN && (g_millis - g_stop_count) > 15000)
		{
			start_avoid_flag = 1;
		}
		...
		if (g_mag_cnt == 4)
		{
			delay_1ms(300); // 整个比赛结束停车前的最后延时，不影响运行逻辑，保留即可
			track_cnt = 0;
...
void TIMER3_IRQHandler(void)
{
    if(SET == timer_interrupt_flag_get(TIMER3, TIMER_INT_FLAG_UP))
    {
        ...
        if(g_round_state == ROUND_NONE)
        {
            if(track_cnt == 1)
            {
                if(has_parked == false && has_bridge == true) base_pwm = 5500;
                else base_pwm = 6000;
                track_PID.P = 50.0f;
            ...
            else if (g_bridge_state == BRIDGE_KEEP && avoid_cnt == 0)
            {
				...
				// 桥的时间判定整合到这里（替代原来的 cross_bridge）
				if(g_millis - bridge_start_ms > 920)
				{
            ...
            else if(avoid_cnt == 1)
            {
                ...
                adc_value[0]=0; adc_value[1]=0; adc_value[2]=1500; adc_value[3]=0; adc_value[4]=1300;
                if(g_millis - avoid_start_ms > 520) avoid_cnt = 2; // 不再清零 g_millis！
                real_error = cal_err();
                positional_PD_track(real_error, 0, &track_PID);
            }
            else if(avoid_cnt == 2)
            {
                ...
                adc_value[0]=3500; adc_value[1]=0; adc_value[2]=1500; adc_value[3]=0; adc_value[4]=0;
                if(g_millis - avoid_start_ms > 750) avoid_cnt = 3;
                real_error = cal_err();
                positional_PD_track(real_error, 0, &track_PID);
            }
            else if(avoid_cnt == 3)
            {
                car_rgb_set_both(red);
                adc_value[0]=0; adc_value[1]=0; adc_value[2]=1500; adc_value[3]=0; adc_value[4]=2500;
                if(g_millis - avoid_start_ms > 500)
                {
                    avoid_cnt = 0;
                    track_cnt = 1;
                    is_checking_obs = 0;
            ...
            else if(parking_cnt == 1)
            {
                ...
                track_PID.P = 40.0f;
                base_pwm = 5500;
                adc_value[0]=800; adc_value[1]=0; adc_value[2]=0; adc_value[3]=0; adc_value[4]=0;
                if(g_millis - park_start_ms > 230) parking_cnt = 2;
                real_error = cal_err();
                positional_PD_track(real_error, 0, &track_PID);
            }
            else if(parking_cnt == 2)
            {
                ...
                base_pwm = 5500;
                adc_value[0]=0; adc_value[1]=0; adc_value[2]=0; adc_value[3]=0; adc_value[4]=2600;
                if(g_millis - park_start_ms > 300) parking_cnt = 3;
                real_error = cal_err();
                positional_PD_track(real_error, 0, &track_PID);
            }
            ...
            else if(parking_cnt == 4)
            {
                car_rgb_set_both(green);
                base_pwm = 6000;
                adc_value[0]=0; adc_value[1]=0; adc_value[2]=0; adc_value[3]=0; adc_value[4]=800;
                if(g_millis - park_start_ms > 280) parking_cnt = 5;
                real_error = cal_err();
                positional_PD_track(real_error, 0, &track_PID);
            }
            ...
            {
                car_rgb_set_both(red);
                adc_value[0]=2500; adc_value[1]=0; adc_value[2]=1500; adc_value[3]=0; adc_value[4]=0;
                if(g_millis - park_start_ms > 400)
                {
                    parking_cnt = 0;
                    has_parked = true;
                    track_cnt = 1;
        ...
        else if(g_round_state == ROUND_IN && has_round_flag == 0)
        {
			...
			// 进环岛的时间判定整合到这里（替代原来的 handle_roundabout）
			if(g_millis - round_start_ms > 400)
			{
...
void check_parking_trigger(void)
{
	...
	if(is_parking_triggering && (g_millis - park_trig_start > 400))
	{
		parking_cnt = 1;
		car_rgb_set_both(red);
		g_flag_cnt[3] = 1;
...
void avoid_obs(void)
{
	...
	if (tof_data_ready != 0)
	{
		...
		if (is_checking_obs && g_total_distance < 850 && g_total_distance > 0 && has_avoided_obs == false)
		{
			// 去掉 g_millis = 0;
			track_cnt = 0;
			avoid_cnt = 1;
