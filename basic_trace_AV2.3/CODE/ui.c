...
void ui_task(void) {
        ...
        switch (ui_page) {
            ...
            case PAGE_PID_PWM: {
                ...
                if (step_pid != 0) {
                    ...
                    else if (ui_cursor == 2) used_speed_duty = ui_adjust_u32(used_speed_duty, step_pid * 500, 0, 10000);
                    else if (ui_cursor == 3) target_speed_duty = ui_adjust_u32(target_speed_duty, step_pid * 1000, 0, 10000);
                    req.values_only = 1;
                }
            ...
            case PAGE_ROUND:
            case PAGE_PARK: {
                uint8_t total = (ui_page == PAGE_AVOID) ? 7 : (ui_page == PAGE_ROUND ? 5 : 9);
                int8_t step;
                ...
                if (step != 0) {
                    if (ui_page == PAGE_AVOID) {
                        ...
                        else if (ui_cursor == 6) ble_avoid_distance_mm = ui_adjust_u16(ble_avoid_distance_mm, step * 50, 0, 4000);
                        ...
                        else if (ui_cursor == 2) round_adc_threshold = ui_adjust_u16(round_adc_threshold, step * 50, 0, 4095);
                        else if (ui_cursor == 3) bridge_keep_time_ms = ui_adjust_u16(bridge_keep_time_ms, step * 10, 0, 5000);
                        else if (ui_cursor == 4) round_check_delay_ms = ui_adjust_u16(round_check_delay_ms, step * 100, 100, 10000);
    ...
    if (req.list_items && (ui_page == PAGE_MAIN_MENU || ui_page == PAGE_AVOID || ui_page == PAGE_ROUND || ui_page == PAGE_PARK)) {
        uint8_t total = (ui_page == PAGE_MAIN_MENU) ? 7 : (ui_page == PAGE_AVOID ? 7 : (ui_page == PAGE_ROUND ? 5 : 9));
        uint8_t i;
        for (i = 0; i < 4; i++) {
            ...
            if (ui_page == PAGE_MAIN_MENU) {
                ...
                else if (item_idx == 6) sprintf(buf, "AvDst: %-5u", ble_avoid_distance_mm);
            } else if (ui_page == PAGE_ROUND) {
                if      (item_idx == 0) sprintf(buf, "T_Rnd: %-5u", t_round);
                else if (item_idx == 1) sprintf(buf, "BrErr: %-5d", bridge_target_error);
                else if (item_idx == 2) sprintf(buf, "RAdc: %-5u", round_adc_threshold);
                else if (item_idx == 3) sprintf(buf, "BrTm: %-5u", bridge_keep_time_ms);
                else if (item_idx == 4) sprintf(buf, "RdDly: %-5u", round_check_delay_ms);
            } else if (ui_page == PAGE_PARK) {
