  ...
  while(1)
	{
		...
		if((has_round_flag == 0) && (is_checking_round == 0) &&
		   ((uint32_t)g_millis >= round_check_delay_ms))
		{
			is_checking_round = 1;
		}
...
static void control_10ms_task(void)
{
        ...
        if(g_round_state == ROUND_NONE)
        {
            ...
            else if (g_bridge_state == BRIDGE_KEEP && avoid_cnt == 0)
            {
				...
				if(g_millis - bridge_start_ms > bridge_keep_time_ms)
				{
...
void check_roundabout_trigger(void)
{
	...
	uint8_t can_check_round;

	can_check_round = (has_round_flag == 0) &&
	                  (is_checking_round == 1) &&
	                  (start_round == 0) &&
	                  (g_round_state == ROUND_NONE) &&
	                  (g_bridge_state == BRIDGE_NONE) &&
    ...
    {
        if (adc_value[2] > round_adc_threshold )
        {
			car_rgb_set_both(red);
	...
	{
		car_rgb_set_both(red);
		if((uint32_t)(g_millis - round_trig_start) > 200U)
		{
			start_round = 1;
			is_round_triggering = false;
		}
...
void avoid_obs(void)
{
	...
	if((is_checking_obs == 0) || (ble_avoid_armed == 0) || (has_avoided_obs != false))
	{
		return;
	}
	...
	if (tof_data_ready != 0)
	{
		...
		if (g_total_distance <= ble_avoid_distance_mm && g_total_distance > 0)
		{
			// 去掉 g_millis = 0;
			ble_avoid_armed = 0;
			start_avoid_flag = 0;
...
void cross_bridge(void)
{
	...
	switch (g_bridge_state)
	{
		case BRIDGE_KEEP:
			...
			if ((uint32_t)(g_millis - bridge_start_ms) > bridge_keep_time_ms)
			{
				g_bridge_state = BRIDGE_NONE;
				track_cnt = 1;
				is_checking_bridge = 0;
