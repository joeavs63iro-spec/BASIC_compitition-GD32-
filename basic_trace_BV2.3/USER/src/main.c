...
#define BLE_PARKING_DELAY_MS    2000U
...
uint16_t t_avoid_1 = 330;
uint16_t t_avoid_2 = 350;
uint16_t t_avoid_3 = 2600;
uint16_t t_avoid_4 = 480;
uint16_t t_avoid_5 = 300;
uint16_t a_avoid_1 = 3500;
uint16_t a_avoid_2 = 2000;
uint16_t a_avoid_4 = 3000;
uint16_t a_avoid_5 = 1500;
uint16_t t_round = 1000;
uint16_t round_adc_threshold = 3000;
uint16_t round_trigger_debounce_ms = 200;
...
//	delay_1ms(1000);
	...
	nvic_irq_enable(TIMER3_IRQn, 15, 0); //TIMER3������ȼ�
  while(1)
	{
...
//        }
		...
		if((start_round == 1) && (has_round_flag == 0))
		{
			...
			if(g_round_state == ROUND_KEEP)
			{
					...
					if(g_millis - keep_start_time > t_round) // 等待�?1 �?
					{
							g_round_state = ROUND_OUT;
							car_rgb_set_both(black);
							round_out_ms = g_millis;
							keep_start_time = 0; // 状态结束，清零计时�?
					}
			}
		}
			// �?main 函数 while(1) 内部替换为：
		...
		if(avoid_cnt == 3)
		{
				...
				if(g_millis - avoid_wait_start > t_avoid_3) // 时间到了 3 �?
				{
						g_millis = 0;
						avoid_cnt = 4;
						ble_avoid_tx_stop();
		...
		else if (avoid_cnt == 0 && (g_millis-g_stop_count)>700&&g_parking_state == PARKING_WAIT)//添加接收到蓝�?x03消息判断条件
		 {
...
static void control_10ms_task(void)
{
 if(g_round_state==ROUND_NONE)
 {
	  ...
	  else if(avoid_cnt==1)
	 {
		 ...
		 adc_value[0]=a_avoid_1;
		 adc_value[1]=0;
		 adc_value[2]=1500;
		 adc_value[3]=0;
		 adc_value[4]=0;
		 if(g_millis>t_avoid_1)
		 {
			 avoid_cnt = 2;
			 g_millis = 0;
		 }
	  ...
	  else if(avoid_cnt == 2)
	 {
		  ...
		  adc_value[2]=1500;
		  adc_value[3]=0;
		  adc_value[4]=a_avoid_2;
		 if(g_millis >= t_avoid_2)
		 {
				 avoid_cnt = 3;
				 g_millis = 0;
		 }
	 ...
	 else if(avoid_cnt==4)
	 {
		  car_rgb_set_both(green);
			...
			adc_value[2]=1500;
			adc_value[3]=0;
			adc_value[4]=a_avoid_4;
		 if(g_millis>t_avoid_4)
		 {
			 avoid_cnt = 5;
			 g_millis = 0;
		 }
	 ...
	 {
			car_rgb_set_both(red);
			adc_value[0]=a_avoid_5;
			adc_value[1]=0;
			adc_value[2]=1500;
			adc_value[3]=0;
			adc_value[4]=0;
			 if(g_millis>t_avoid_5)
			 {
				 avoid_cnt = 0;
				 track_cnt = 1;
				 g_millis = 0;
	...
	{
		motor_forward(right, (uint16_t)target_speed_duty);
		motor_forward(left, (uint16_t)target_speed_duty);
		car_rgb_set_both(green);
		if(g_millis>100)
		{
			  base_pwm = (int)target_speed_duty;
...
void check_roundabout_trigger(void)
{
    ...
    if (start_round == 0)
    {
        if (adc_value[2] > round_adc_threshold && !is_triggering)
        {
            car_rgb_set_both(blue);
            trigger_start_time = g_millis;
            is_triggering = true; // 开始倒计�?
        }
        
        // Round trigger debounce elapsed.
        if (is_triggering && (g_millis - trigger_start_time > round_trigger_debounce_ms))
        {
            start_round = 1;
            g_millis = 0;
