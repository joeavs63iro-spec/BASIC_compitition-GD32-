...
//        }
		...
		if (avoid_cnt == 0 && g_bridge_state != BRIDGE_NONE && is_checking_bridge == 1)
		{
		 ...
		 else if(avoid_cnt == 0&&g_parking_state == PARKING_IN&&(g_millis-g_stop_count)>12000)
		 {
				start_avoid_flag=1;
		 }
//		if (g_mag_cnt == 3)
...
{
	if (avoid_cnt == 0 && g_mag_cnt == 1 && g_flag_cnt[1] == 0)
	{
		track_cnt = 0;
		car_rgb_set_both(yellow);
...
void check_roundabout_trigger(void)
{
    if (start_round == 0)
    {
        ...
        {
					round_flag++;
					if(adc_value[2] > 3000 &&round_flag>20)
					{
						car_rgb_set_both(blue);
						delay_1ms(150);
						start_round = 1;
