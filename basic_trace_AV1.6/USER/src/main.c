...
//		}
		...
		if(parking_cnt == 3 && is_checking_parking == 1)
		{
			...
			if(g_millis - parking_wait_start > 4000)
			{
		...
		if (g_mag_cnt == 4)
		{
			delay_1ms(350); // 整个比赛结束停车前的最后延时，不影响运行逻辑，保留即可
			track_cnt = 0;
    ...
    if(SET == timer_interrupt_flag_get(TIMER3, TIMER_INT_FLAG_UP))
    {
        ...
        if(g_round_state == ROUND_NONE)
        {
            ...
            else if(avoid_cnt == 1)
            {
                ...
                if(g_millis - avoid_start_ms > 450) avoid_cnt = 2; // 不再清零 g_millis！
                real_error = cal_err();
                positional_PD_track(real_error, 0, &track_PID);
            }
            else if(avoid_cnt == 2)
            {
                ...
                if(g_millis - avoid_start_ms > 650) avoid_cnt = 3;
                real_error = cal_err();
                positional_PD_track(real_error, 0, &track_PID);
            }
            else if(avoid_cnt == 3)
            ...
            else if(parking_cnt == 1)
            {
                ...
                track_PID.P = 40.0f;
                base_pwm = 5500;
                adc_value[0]=1200; adc_value[1]=0; adc_value[2]=0; adc_value[3]=0; adc_value[4]=0;
                if(g_millis - park_start_ms > 300) parking_cnt = 2;
                real_error = cal_err();
                positional_PD_track(real_error, 0, &track_PID);
            }
            ...
            else if(parking_cnt == 4)
            {
                car_rgb_set_both(green);
                base_pwm = 6000;
                adc_value[0]=0; adc_value[1]=0; adc_value[2]=0; adc_value[3]=0; adc_value[4]=1200;
                if(g_millis - park_start_ms > 220) parking_cnt = 5;
                real_error = cal_err();
                positional_PD_track(real_error, 0, &track_PID);
            }
...
void check_parking_trigger(void)
{
	...
	if(is_parking_triggering && (g_millis - park_trig_start > 380))
	{
		parking_cnt = 1;
		car_rgb_set_both(red);
		g_flag_cnt[3] = 1;
...
void avoid_obs(void)
{
	...
	if (tof_data_ready != 0)
	{
		...
		if (is_checking_obs && g_total_distance < 800 && g_total_distance > 0 && has_avoided_obs == false)
		{
			// 去掉 g_millis = 0;
			track_cnt = 0;
			avoid_cnt = 1;
