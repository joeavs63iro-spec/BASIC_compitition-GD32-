 ...
 *
 * @file       dmx_tof400c.c
 * @brief      呆萌侠GD32F303RC开源库
 * @company    合肥呆萌侠智能科技有限公司
 * @author     呆萌侠科技（QQ：2453520483）
 * @MCUcore    GD32F303RC
 * @Software   MDK ARM_V6.19
...
void Init_TOF400C(void)
{
    ...
    // 初始化超时
    unsigned int time = TOF400C_TIMEOUT_COUNT * 100;

	  gpio_pin_init(TOF400C_SCL_PORT, OUT_PP, TOF400C_SCL_PIN);
	  gpio_pin_init(TOF400C_SDA_PORT, OUT_OD, TOF400C_SDA_PIN);
	  TOF400C_SCL_LEVEL(1);
	  TOF400C_SDA_LEVEL(1);
	  delay_1ms(2);
	
	
//    rcu_periph_clock_enable(TOF400C_GPIO_CLK);

//    gpio_init(TOF400C_SCL_PORT, GPIO_MODE_OUT_PP, GPIO_OSPEED_50MHZ, TOF400C_SCL_PIN);
//    gpio_init(TOF400C_SDA_PORT, GPIO_MODE_OUT_OD, GPIO_OSPEED_50MHZ, TOF400C_SDA_PIN);
	
    TOF400C_Read_Word_Data(VL53L1_IDENTIFICATION__MODEL_ID, &sensor_id);
    if(sensor_id != VL53L1X_EXPECTED_MODEL_ID)
    {
        printf("TOF400C ID Error: 0x%04X\r\n", sensor_id);
    }

    /* This vendor table is a full 0x0001..0x0087 register image. */
    ...
    /* Do not rewrite the configuration while waiting: that restarts ranging. */
    while((tmp == 0) && (time > 0))
    {
        tmp = Check_TOF400C();
        time--;
...
{
    unsigned char range_status = 0;
    unsigned int tof400c_distance = 1400;

    /* Only consume and clear a completed measurement. */
    if(Check_TOF400C() == 0)
        return tof400c_distance;

    TOF400C_Read_Data(RESULT__RANGE_STATUS, &range_status);
    /* The valid status is 9 in the low five bits; raw data may be 0x09 or 0x89. */
    if((range_status & 0x1F) == 9)
    {
        TOF400C_Read_Word_Data(VL53L1_RESULT__FINAL_CROSSTALK_CORRECTED_RANGE_MM_SD0, &tof400c_distance);
        if(tof400c_distance > 1400 )
...
static int TOF400C_Waitack(void)
{
    ...
    TOF400C_SCL_LEVEL(1);
    TOF400C_DELAY_MS(TOF400C_DELAY);
    if(gpio_get_input(TOF400C_SDA_PORT , TOF400C_SDA_PIN))
    {
        TOF400C_SCL_LEVEL(0);
        TOF400C_DELAY_MS(TOF400C_DELAY);
        return 0;
