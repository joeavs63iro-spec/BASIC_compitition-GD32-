...
static void control_10ms_task(void)
{
        ...
        if(g_round_state == ROUND_NONE)
        {
            ...
            else if(avoid_cnt == 2)
            {
                ...
                if(g_millis - avoid_start_ms > 560) avoid_cnt = 3;
                real_error = cal_err();
                positional_PD_track(real_error, 0, &track_PID);
            }
            else if(avoid_cnt == 3)
            {
                ...
                if(g_millis - avoid_start_ms > 350)
                {
                    avoid_cnt = 0;
                    track_cnt = 1;
                    is_checking_obs = 0;
            ...
            else if(parking_cnt == 1)
            {
                ...
                track_PID.P = 40.0f;
                base_pwm = 5500;
                adc_value[0]=2100; adc_value[1]=0; adc_value[2]=0; adc_value[3]=0; adc_value[4]=0;
                if(g_millis - park_start_ms > 260) parking_cnt = 2;
                real_error = cal_err();
                positional_PD_track(real_error, 0, &track_PID);
            }
            else if(parking_cnt == 2)
            {
                ...
                if(g_millis - park_start_ms > 260) parking_cnt = 3;
                real_error = cal_err();
                positional_PD_track(real_error, 0, &track_PID);
            }
