...
//  timer_pit_interrupt_ms(TIMER3,10);
    // TOF distance is triggered by PA12 external interrupt.
    // timer_pit_interrupt_ms(TIMER4,50);
    // nvic_irq_enable(TIMER4_IRQn, 14, 0);
    ...
    exti_enable(GPIOA, GPIO_PIN_12, FALLING);
    gpio_pin_init(GPIOA, IN_PULLUP, GPIO_PIN_12); // TOF INT: PA12
