...
#define XUNXIAN_DUTY_MAX        7000
#define XUNXIAN_DUTY_DEADBAND   1400
#define XUNXIAN_LOST_RAW_SUM    50.0f
#define XUNXIAN_STEER_DIR       (-1.0f)

static unsigned int limit_motor_duty(long int duty)
{
    if(duty <= 0)
    {
        return 0;
    }
    if(duty < XUNXIAN_DUTY_DEADBAND)
    {
        return 0;
    }
    if(duty > XUNXIAN_DUTY_MAX)
    {
        return XUNXIAN_DUTY_MAX;
    }
...
void diancixunxian()// core line-following control
{
    ...
    raw_sum = adc_value[0] + adc_value[1] + adc_value[2] + adc_value[3] + adc_value[4];

    if((raw_sum < XUNXIAN_LOST_RAW_SUM) || (signal_sum <= 0.1f))
    {
        left_motor_duty = 0;
        right_motor_duty = 0;
        error[0] = 0;
        oled_show_string(10,7,"LOS",12);
        car_stop();
    }
    else
    {
        error[0] = XUNXIAN_STEER_DIR * 100.0f * ((2.0f * rr + rm) - (lm + 2.0f * ll)) / signal_sum;
        er_speed = (long int)(error[0] * kp * 7.0f + (error[0] - error[1]) * kd * 7.0f);

        left_motor_duty = limit_motor_duty((long int)st_speed + er_speed);
        right_motor_duty = limit_motor_duty((long int)st_speed - er_speed);
        oled_show_string(10,7,"TRK",12);
    }
...
{
			oled_show_number(30,1,adc_value[0],4,12);
			oled_show_number(30,2,adc_value[1],4,12);
			oled_show_number(30,3,adc_value[2],4,12);
			oled_show_number(30,4,adc_value[3],4,12);
			oled_show_number(30,5,adc_value[4],4,12);
}
void oled_show_gyh() //屏幕显示归一化并且滤波的ADC值  
{
			oled_show_number(5,1,ll,4,12);
			oled_show_number(5,2,lm,4,12);
			oled_show_number(5,3,mm,4,12);
			oled_show_number(5,4,rm,4,12);
			oled_show_number(5,5,rr,4,12);
}
void xunxian() //巡线
{
