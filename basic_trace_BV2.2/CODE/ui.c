...
void ui_task(void) {
        ...
        switch (ui_page) {
            ...
            case PAGE_MAIN_MENU: {
                ...
                if (key == KEY5_PRESS || key == KEY3_PRESS) {
                    ...
                    else if (ui_cursor == 5) ui_page = PAGE_SENSOR_ALL;
            ...
            case PAGE_AVOID:
            case PAGE_ROUND: {
                uint8_t total = (ui_page == PAGE_AVOID) ? 9 : 3;
                int8_t step;
                ...
                if (key == KEY5_PRESS) { ui_save_and_return((ui_page==PAGE_AVOID)?3:4); }
                break;
            }
            case PAGE_SENSOR_ALL:
                if (key == KEY5_PRESS || key == KEY4_PRESS) { ui_page = PAGE_MAIN_MENU; ui_cursor = 5; scroll_top = 2; req.full_page = 1; }
                break;
        }
    }
    ...
    if (req.list_items && (ui_page == PAGE_MAIN_MENU || ui_page == PAGE_AVOID || ui_page == PAGE_ROUND)) {
        uint8_t total = (ui_page == PAGE_MAIN_MENU) ? 6 : (ui_page == PAGE_AVOID ? 9 : 3);
        uint8_t i;
        for (i = 0; i < 4; i++) {
            ...
            if (ui_page == PAGE_MAIN_MENU) {
                ...
                else if (item_idx == 4) sprintf(buf, "5.Round/Speed");
                else if (item_idx == 5) sprintf(buf, "6.All Sensors");
