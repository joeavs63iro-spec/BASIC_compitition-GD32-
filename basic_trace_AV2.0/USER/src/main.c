...
void ai_wb2_01s_ch9143_rx_byte(uint8_t data)
{
	...
	if((ch9143_rx_len >= 4U) ||
	   ((ch9143_rx_len >= 2U) && (ch9143_rx_buffer[0] == '0') && (ch9143_rx_buffer[1] == '1')))
	{
		ch9143_packet_ready = 1;
	}
}
...
static void ble_avoid_recv_task(void)
{
...
//		}
		...
		if (g_mag_cnt == 4)
		{
			delay_1ms(450); // 整个比赛结束停车前的最后延时，不影响运行逻辑，保留即可
			track_cnt = 0;
...
static void control_10ms_task(void)
{
        ...
        if(g_round_state == ROUND_NONE)
        {
            ...
            else if(avoid_cnt == 2)
            {
                ...
                if(g_millis - avoid_start_ms > 600) avoid_cnt = 3;
                real_error = cal_err();
                positional_PD_track(real_error, 0, &track_PID);
            }
            else if(avoid_cnt == 3)
            ...
            else if(parking_cnt == 1)
            {
                ...
                track_PID.P = 40.0f;
                base_pwm = 5500;
                adc_value[0]=2000; adc_value[1]=0; adc_value[2]=0; adc_value[3]=0; adc_value[4]=0;
                if(g_millis - park_start_ms > 265) parking_cnt = 2;
                real_error = cal_err();
                positional_PD_track(real_error, 0, &track_PID);
            }
            else if(parking_cnt == 2)
            {
                ...
                if(g_millis - park_start_ms > 270) parking_cnt = 3;
                real_error = cal_err();
                positional_PD_track(real_error, 0, &track_PID);
            }
            ...
            else if(parking_cnt == 4)
            {
                car_rgb_set_both(green);
                base_pwm = 6000;
                adc_value[0]=0; adc_value[1]=0; adc_value[2]=0; adc_value[3]=0; adc_value[4]=1800;
                if(g_millis - park_start_ms > 200) parking_cnt = 5;
                real_error = cal_err();
                positional_PD_track(real_error, 0, &track_PID);
            }
...
void check_parking_trigger(void)
{
	...
	if(is_parking_triggering && (g_millis - park_trig_start > 390))
	{
		track_cnt = 0;
		parking_cnt = 1;
		car_rgb_set_both(red);
